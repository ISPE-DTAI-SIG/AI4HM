# AI4HM — Protocol Template

**AI4HM** (AI/ML for Health and Medicine) is a fit-for-purpose study protocol template that provides
structure while preserving methodological flexibility — accommodating iterative model development,
feature engineering as part of the research process, multiple validation stages, performance metrics
beyond traditional epidemiology, and fairness, bias, and interpretability considerations.

This repository hosts the interactive web version of the template, published via GitHub Pages.

## 🔗 Live page

**https://ispe-dtai-sig.github.io/AI4HM/**

## 📄 Download the template

A Microsoft Word version (`.docx`, v0.2) is available from the **Download .docx** button on the live
page, or directly at [`assets/AI4HM-Protocol-Template-v0.2.docx`](assets/AI4HM-Protocol-Template-v0.2.docx).

## Repository contents

| File | Description |
| --- | --- |
| `index.html` | Self-contained interactive protocol template (single file; renders the full docs). |
| `assets/AI4HM-Protocol-Template-v0.2.docx` | Downloadable Word version of the template. |

## About

Developed by the **ISPE Digital Technology & AI Special Interest Group (DTAI SIG)** as part of the
AI4HM authoring effort to support reproducible, transparent reporting of AI/ML studies in health and
medicine.

---

*The interactive page is a single self-contained HTML file. To preview locally, open `index.html`
in a browser, or serve the folder (e.g. `python3 -m http.server`) so the document download resolves.*
